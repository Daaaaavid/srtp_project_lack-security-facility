		...
		try{
			// 加载数据库驱动，注册到驱动管理器
						...
						// 获取Statement
							...
							// 添加图书信息的SQL语句
							String sql = "select * from tb_guanliyuan";
							// 执行查询
							ResultSet rs = stmt.executeQuery("select * from tb_guanliyuan where id='"+name+"'");
			if(rs.next()){
				rs=stmt.executeQuery("select * from tb_guanliyuan where id='"+name+"' and password='"+password+"'");
				if(rs.next()){
					request.getRequestDispatcher("login_success_guanliyuan.jsp").forward(request,response);
				}else{
					%>
			    ...
			    <div style=line-height:600px; margin:30px;">
			    <font color=blueviolet size=50px>用户不存在！请检查管理员id。</font>
			    </div>
